...
export function SellerList({ initialSellers }: SellerListProps) {
    // Usa useLiveQuery para reagir a mudanças locais (offline)
    const localSellers = useLiveQuery(() => db.sellers.toArray());
    const sellers = (localSellers && localSellers.length > 0) ? localSellers : initialSellers;
            ...
            <CardContent className="p-0">
                ...
                <StaggerContainer className="grid grid-cols-1 gap-4 md:hidden p-4">
                    {sellers.map((seller: Seller) => {
                        const totalQty = (seller.consignments || []).reduce((acc: number, c: any) => acc + (c.quantity || 0), 0);
                        const totalValue = (seller.consignments || []).reduce((acc: number, c: any) => acc + ((c.quantity || 0) * (c.product?.price || 0)), 0);
